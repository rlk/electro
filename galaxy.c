...
void galaxy_draw(const double p[3])
{
    ...
    {
        float dx = S[i].position[0] - p[0];
        float dy = S[i].position[1] - p[1];
        float dz = S[i].position[2] - p[2];
        float d  = dx * dx + dy * dy + dz * dz;
        float m  = S[i].magnitude + log_2(d / 100.0) * 0.7525749;
        float L  = pow(10.0, -m / 5.0);

        glPointSize(L * 50);
        glBegin(GL_POINTS);
        glVertex3fv(S[i].position);
        glEnd();
    }
}
*/
