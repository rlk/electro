...
function set_positions()
    ...
    if that_image then
        x0, y0, z0, x1, y1, z1 = E.get_entity_bound(that_sprite_L)
    else
        x0, y0, z0, x1, y1, z1 = E.get_entity_bound(this_sprite_L)
    end
    ...
    E.set_entity_position(that_sprite_L, pos_x + off_x,
                                         pos_y + off_y,
                                         pos_z + off_z)
    E.set_entity_position(that_sprite_R, pos_x - off_x, 
                                         pos_y - off_y,
                                         pos_z + off_z)

    -- Position the border mask

    local k = math.min((X1 - X0 - math.abs(off_x) * 2) / (X1 - X0),
                       (Y1 - Y0 - math.abs(off_y) * 2) / (Y1 - Y0))

    print(off_x, (X1 - X0), k)

    E.set_entity_scale(mask, s * k, s * k, s * k)
    E.set_entity_position(mask, pos_x, pos_y, pos_z + off_z)

    -- Position the inset view
    ...
    E.set_entity_position(this_sprite_L, pos_x,
                                         pos_y,
                                         pos_z + off_z)
    E.set_entity_position(this_sprite_R, pos_x,
                                         pos_y,
                                         pos_z + off_z)
...
function do_start()
    ...
    mask    = E.create_object("mask.obj")

    E.set_brush_flags(E.get_mesh(mask, 0), E.brush_flag_unlit, true)
